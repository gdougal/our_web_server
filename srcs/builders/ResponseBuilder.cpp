...
string ResponseBuilder::build_response(methods qurey_type) {
    ...
    if (path_res.c_str()[path_res.length() - 1] == '/' && r->autoindex)
        body = AutoindexResonseBuilder().build(serverConfig, "/home/lmallado/Desktop/our_web_server" + path_res);
    else
...
string ResponseBuilder::read_from_file(string path_res) {
    ifstream page("/home/lmallado/Desktop/our_web_server" + path_res);
    string tmp;
    string body;
